...
app.post("/api/v1/auth/register", async (req, res) => {
    try {
        ...
        if (!isStrongPassword) {
            return res
                .status(400)
                .json({ message: "Password must be stronger" });
        }
...
app.get("/api/docs", (req, res, next) => {
    try {
        res.redirect(
            "https://documenter.getpostman.com/view/20451580/2s9Y5YSi6s"
        );
    } catch (error) {
        console.error(error);
    ...
    try {
        ...
        const data = await Promise.all(
            response.data.results.map(async (dt) => {
                const response = await axios.get(
                    `https://api.themoviedb.org/3/movie/${dt.id}/videos?api_key=${API_KEY}`,
                    { httpsAgent: agent }
                );

                dt.videos = response.data.results;
                return dt;
            })
        );
...
app.get("/api/v1/movie/:movie_id", authorization, async (req, res, next) => {
    try {
        ...
        data.videos = responseVideos.data.results;
